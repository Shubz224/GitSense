...
export async function summariseCode(doc: Document) {
    console.log("getting summmary for", doc.metadata.source);
    try {
        const code = doc.pageContent.slice(0, 10000);
        const response = await model.generateContent([
            `you are an intelligent senior software engineer who specialises in onboarding junior software enginners onto projects
